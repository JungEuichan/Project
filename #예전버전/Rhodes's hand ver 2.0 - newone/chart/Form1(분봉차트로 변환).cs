...
namespace chart
{
    public partial class Form1 : Form
    {
        ...
        string sellStockName;           //사용자가 balancesheet에서 마우스 클릭을 통해 선택한 종목명, 보유종목들
        ...
        private void timer1_tick(object sender, EventArgs e) //조건검색 종목을 1초당 하나씩 조회요청한다. / 누적호가,체결데이터를 리셋 / 체결데이터 그리드업데이트 / 선 자동분류
        {
            ...
            if (notifierCheck.Checked == true)
            {
                for (int i = 0; i < listBox1.Items.Count; i++)
                {
                    if (listBox1.GetItemChecked(i) == false)
                    {
                        playSimpleSound();
                    }
                }
            ...
            //3분이 지났다면, 누적체결량/ 누적순매수 / 누적순매도 값을 리셋함. + sec값도 0으로 바꿔줌.
            if (sec > 180)
            {
                sec = 0;
            ...
            if (sec % 3 == 0)
            {
                for (int j = 0; j < dataGridView.Rows.Count; j++)
                {
                    ...
                    if (i > -1)
                    {
                        ...
                        dataGridView.Rows[j].Cells[5].Value = stockDB[i].상승확률;
                    }
                }
            }
                ...
                if (i >= 0)
                {
                    if (long.Parse(axKHOpenAPI1.GetCommData(e.sTrCode, e.sRQName, 0, "거래량").Trim()) > 0)
                    {
                        ...
                        for (int j = 490; j >= 0; j--)
                        {
                            ...
                            time = long.Parse(axKHOpenAPI1.GetCommData(e.sTrCode, e.sRQName, j, "체결시간").Trim());

                            stockDB[i].priceAdd(h, l, open, close, v, time);
                        }
        ...
        public void onReceiveRealCondition(object sender, AxKHOpenAPILib._DKHOpenAPIEvents_OnReceiveRealConditionEvent e) //실시간 조건검색결과 이벤트함수 / 종목추가시 stockDB에 추가, 종목개수 라벨에 update
        {
            if (e.strType == "I") //종목 편입
            {
                ...
                int i = stockDB.FindIndex(o => o.stockCode == e.sTrCode);
                if (i == -1)
                {
                    stockDB.Add(new stockInfo(e.sTrCode, stockName));
        ...
        private void passwordBox_TextChanged(object sender, EventArgs e) //계좌비밀번호를 입력했을때, 잔고요청을 하며 timer2를 활성화하여 잔고현황을 실시간으로 업데이트한다.
        {
            잔고요청();
            ...
            if (passwordBox.TextLength == 4 && accountBox.Text.Length > 0)
            {
                ...
                for (int i = 0; i < balanceDataGridView.Rows.Count; i++)
                {
                    ...
                    if(j>-1)
                    {
                        ...
                        balanceDataGridView.Rows[i].Cells[3].Value = stockBalanceList[i].손익율;
                        balanceDataGridView.Rows[i].Cells[4].Value = stockBalanceList[i].최고손익율;
                        balanceDataGridView.Rows[i].Cells[5].Value = stockBalanceList[i].현재가;
                        balanceDataGridView.Rows[i].Cells[6].Value = stockBalanceList[i].감시상한가;
                        balanceDataGridView.Rows[i].Cells[7].Value = stockBalanceList[i].감시하한가;
                    ...
                    if (자동매도Check.Checked == true && stockBalanceList[i].손익율 > -30)
                    {
                        ...
                        if((감시가check.Checked == true && stockBalanceList[i].열람값 == true) && (stockBalanceList[i].현재가 > stockBalanceList[i].감시상한가 || stockBalanceList[i].현재가 < stockBalanceList[i].감시하한가))
                        {
                            stockBalanceList[i].열람값 = false;
                            sell(stockBalanceList[i]);
                        }
        ...
        public void 분봉조회(string stockCode)
        {
            ...
            addToLogBox(": 분봉데이터를 요청합니다. 종목코드:" + stockCode + "화면번호 : " + (5100 + scrNum_TR));
            scrNum_TR++;
        }
        ...
        public void renewal()
        {
            for (int i = 0; i < stockDB.Count; i++)
            {
                ...
                // 1. 거래량 합산 및 최저 거래량을 돌리고, 최저거래량보다 거래량이 큰상태인 애를 location1로 지정한다.

                // 2. 추가적으로 l선위에, h선 아래에 있는 애들을 골라서 listbox0에 표시한다. > 얘는 구현하지 말자.


                stockDB[i].distribute();
        ...
        public void Present()
        {
            ...
            if (i >= 0)
            {
                ...
                PriceLogListBox.Items.Add("lLine :" + stockDB[i].lLine);
                PriceLogListBox.Items.Add("금일저가 :" + stockDB[i].금일저가);
                PriceLogListBox.Items.Add("금일고가 :" + stockDB[i].금일고가);

                PriceLogListBox.Items.Add("금일 첫분봉 인덱스 :" + stockDB[i].todayBong());

                for (int j = 0; j <= stockDB[i].종가.Count - 1; j++)
                {
                    PriceLogListBox.Items.Add(j + "번째 : (종)" + stockDB[i].종가[j] + " (시)" + stockDB[i].시가[j] + " (저)" + stockDB[i].저가[j] + " (고)" + stockDB[i].고가[j] + "(거래량)" + stockDB[i].거래량[j] + "(시간)"  + stockDB[i].시간[j]);
                }
            }
        }    
        ...
        private void balanceDataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e) //보유종목 잔고에서 종목선택시 라벨에 종목을 표시해 포커스를 두어 나중에 매도할 준비를 한다..
        {
            ...
            sellStockName = balanceDataGridView.Rows[i].Cells[0].Value.ToString();
        ...
        private void textFileButton_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < listBox1.Items.Count; i++)
            {
                string stockName = listBox1.Items[i].ToString().Split(';')[0];
                string stockCode = stockDB.Find(o => o.stockName == stockName).stockCode;
        ...
        private void ListBoxClick(object sender, EventArgs e) //사용자 편의를 위해 listbox더블클릭시 종목코드를 자동으로 복사해준다.
        {
            int i = listBox1.SelectedIndex;
            if(i>-1)
            {
                int j = stockDB.FindIndex(o => o.stockName == listBox1.Items[i].ToString().Split(';')[0]);
                if (j > -1)
                {
                    Clipboard.SetText(stockDB[j].stockCode);

                    System.Windows.Forms.Cursor.Position = new Point(2222, 125);
                    ...
                    stockDB[j].check = true;
                }
            }

        }

        private void 적용_Click(object sender, EventArgs e)
        {
            int i = stockBalanceList.FindIndex(o => o.stockName == sellStockName);
            int result = 0;

            if(i>-1)
            {
                if (UpThresPrice.TextLength > 0)
                {
                    Int32.TryParse(UpThresPrice.Text, out result);
                    stockBalanceList[i].감시상한가 = result;
                }

                if (DownThresPrice.TextLength > 0)
                {
                    Int32.TryParse(DownThresPrice.Text, out result);
                    stockBalanceList[i].감시하한가 = result;
                }
            }
