    ...
    public partial class Form1 : Form
    {
        ...
        public void onEventConnect(object sender, AxKHOpenAPILib._DKHOpenAPIEvents_OnEventConnectEvent e) //로그인 이벤트 함수. Form1형성후 한번만 호출된다. 
        {
            if (e.nErrCode == 0)
            {
                ...
                // ERROR 조심. 사용자 계좌에 있는 종목이 코스피 가격을 받아오는 이슈가 있음. 시간차를 두고 받아오던가 해야 할듯.
                ...
                if (i >= 0)
                {
                    if (long.Parse(axKHOpenAPI1.GetCommData(e.sTrCode, e.sRQName, 0, "거래량").Trim()) > 0)
                    {
                        ...
                        for (int j = 390; j >= 1; j--)
                        {
                            ...
                            if (stockDB[i].ma5 < close)
                            {
                                stockDB[i].MBuySum += close * v;
                                stockDB[i].vMBuySum += v;
                            }
                            ...
                            if (stockDB[i].ma5 > close)
                            {
                                stockDB[i].MSellSum += l * v;
                                stockDB[i].vMSellSum += v;
                            }
                        }
                        ...
                        long MLLine = stockDB[i].MSellSum / stockDB[i].vMSellSum;
                        long MHLine = stockDB[i].MBuySum / stockDB[i].vMBuySum;

                        stockDB[i].lineAdd(Convert.ToInt32(lLine), Convert.ToInt32(hLine), Convert.ToInt32(MLLine), Convert.ToInt32(MHLine));
        ...
        private void timer1_tick(object sender, EventArgs e) //조건검색 종목을 1초당 하나씩 조회요청한다. / 누적호가,체결데이터를 리셋 / 체결데이터 그리드업데이트 / 선 자동분류
        {
            ...
            //체크되지 않은 새로운 종목이 있다면 알람을 3초마다 울려준다. 
            if (notifierCheck.Checked == true && sec % 6 == 0)
            {
        ...
        public Boolean lineDiff(int line, int price)
        {
            const double d = 0.007;

            if (Math.Abs(((double)line - (double)price) / (double)line) < d)
                return true;
            else
                return false;
        }
        ...
        public void renewal()
        {
            for (int i = 0; i < stockDB.Count; i++)
            {
                ...
                if (stockDB[i].HLine == 9999999)
                {
                    stockDB[i].location = 4;
                }
                else if (stockDB[i].HLine == 0)
                {
                ...
                else
                {
                    switch (c)
                    {
                        case int n when (lineDiff(stockDB[i].HLine, n) || lineDiff(stockDB[i].MHLine, n)):
                            stockDB[i].location = 1;
                            break;

                        case int n when (lineDiff(stockDB[i].lLine, n) || lineDiff(stockDB[i].MLLine, n)):
        ...
        public void Present()
        {
            ...
            if (i >= 0)
            {
                ...
                PriceLogListBox.Items.Add("MhLine :" + stockDB[i].MHLine);
                PriceLogListBox.Items.Add("MlLine :" + stockDB[i].MLLine);
                PriceLogListBox.Items.Add("LLine :" + stockDB[i].lLine);
                PriceLogListBox.Items.Add("ma5 :" + stockDB[i].ma5);
        ...
        private void ListBox1Click(object sender, EventArgs e) //사용자 편의를 위해 listbox더블클릭시 종목코드를 자동으로 복사해준다.	
        {
            int i = listBox1.SelectedIndex;
            if (i > -1)
            {
                ...
                if (j > -1)
                {
                    ...
                    System.Windows.Forms.Cursor.Position = new Point(2222 + 160, 125);
        ...
        private void ListBox2Click(object sender, EventArgs e) //사용자 편의를 위해 listbox더블클릭시 종목코드를 자동으로 복사해준다.	
        {
            int i = listBox2.SelectedIndex;
            if (i > -1)
            {
                int j = stockDB.FindIndex(o => o.stockName == listBox2.Items[i].ToString().Split(';')[0]);
                if (j > -1)
                {
                    Clipboard.SetText(stockDB[j].stockCode);
                    System.Windows.Forms.Cursor.Position = new Point(2222 + 160, 125);
        ...
        {
            int i = stockBalanceList.FindIndex(o => o.stockName == searchNameLabel2.Text);
